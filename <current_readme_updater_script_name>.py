    ...
    while i < len(content):
        ...
        if line.strip() == section_header.strip():
            ...
            if not entry_found:
                ...
                # Adjust 'i' to account for the inserted lines if insertion happened within the current iteration's scan
                # This is tricky because `new_content` is being modified while `i` iterates `content`.
                # A simpler approach for the loop would be to rebuild `new_content` completely after finding the insertion point.
                # For now, let's assume `i` mainly points to `content` and we're inserting into `new_content`
                # and adjust `i` based on whether we added something *after* the current `i`.
                # If we've inserted, we effectively "consumed" the current `i` (the section header) and added more.
                # The next iteration will pick up from where `i` left off in the original `content`.
                # We need to make sure `i` does not re-process the newly added lines as part of `content`.
                # Since we are adding to `new_content` and `i` controls iteration of `content`, `i` does not need adjustment related to `new_content`'s growth for the `while i < len(content)` loop.
                # The crucial part is that `entry_found` is set to True.
...
def update_readme_for_binary_to_decimal_function():
    """Updates README.md with documentation for the 'binary_to_decimal' function."""
    readme_path = "README.md"
    section_header = "## Functions"
    entry_prefix = "###"
    entry_markdown = """
...
*   **Inputs**: `binary_str` (str): A string representing a binary number (e.g., "1011").
*   **Outputs**: (int): The decimal equivalent of the binary string.
*   **Raises**: `ValueError` if the input string is not a valid binary string.
*   **Example**: `binary_to_decimal("1011")` returns `11`.
"""
    _update_readme_section(readme_path, section_header, entry_markdown, entry_prefix)
...
if __name__ == "__main__":
    ...
    try:
        ...
        update_readme_for_binary_to_decimal_function() # New function documentation
